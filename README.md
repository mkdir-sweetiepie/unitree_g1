...
- **전체 Arm Actions**: 16개
- **조이스틱 매핑**: 10개 (buttons 6 + axes 4)
- **코드 전용**: 6개
- **C++ 라이브러리**: 2개 (loco 5.3MB, arm 5.2MB)
- **Python 모듈**: 7개

### 🎯 사용 방법 요약

1. **조이스틱으로 제어**: buttons/axes 입력
2. **프로그래밍으로 제어**: `controller.arm_wave()` 등
3. **범용 메소드**: `controller.arm_action("high_wave")`
4. **동시 제어**: loco와 arm 독립 실행

---

## 📝 라이선스

이 프로젝트는 Unitree SDK2의 라이선스를 따릅니다.
...
- **v1.0**: Loco 제어 구현
- **v2.0**: Arm 제어 추가
- **v3.0**: Loco + Arm 통합 완료 (2025-10-10)
